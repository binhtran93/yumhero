...
<div class="relative {className}" use:clickOutside>
    <button
        ...
        class="p-2 text-app-text-muted hover:text-app-text hover:bg-app-bg rounded-full transition-colors relative"
    ...
    >
        ...
        {#if activeFilter !== "All"}
            <span
                class="absolute top-1 right-1 w-2.5 h-2.5 bg-app-primary rounded-full border-2 border-bg-surface"
            ></span>
        {/if}
    </button>
    ...
    {#if isOpen}
        ...
        >
            ...
            <div class="max-h-64 overflow-y-auto">
                <button
                    class="w-full text-left px-4 py-2 text-sm text-app-text hover:bg-app-surface-hover flex items-center justify-between transition-colors"
                    onclick={() => {
                        activeFilter = "All";
                        closeDropdown();
                    }}
